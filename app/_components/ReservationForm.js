...
function ReservationForm({ cabin, user }) {
  ...
  const totalPrice = Number(regularPrice - discount);
  const numNights = differenceInDays(endDate, startDate);
  const cabinPrice = numNights * totalPrice;
  const bookingData = {
    startDate,
    regularPrice,
    endDate,
  ...
  return (
    <div className="scale-[1.01] order-2">
      ...
      {range?.to && (
        <p className="bg-primary-600 px-5 py-4">
